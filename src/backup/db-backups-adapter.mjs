...
export async function withBundleCredentials(bundle, operation) {
  const profile = {
    ...bundle,
    ...(bundle?.credentials ?? {}),
    host: bundle?.host ?? bundle?.routes?.primary?.[0]?.host,
    port: bundle?.port ?? bundle?.routes?.primary?.[0]?.port,
  };
  if (
    !profile.username ||
    !profile.password ||
    ...
    throw new TypeError("connection bundle credentials are required");
  ...
  try {
    return await operation(file, profile);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
}

export function createBackupFromBundle({
  bundle,
  backupRoot,
  databases,
  metadata,
  ...
  return withBundleCredentials(bundle, async (credentialsFile, profile) => {
    ...
    await (metadata
      ? writeFile(
          join(result.path, "SUPERVISOR-METADATA.json"),
...
export function verifyBackupFromBundle({
  bundle,
  backupPath,
  databases,
  ...options
}) {
  return withBundleCredentials(bundle, (credentialsFile, profile) =>
    verifyBackup({
      backupPath,
      databases,
      credentialsFile,
