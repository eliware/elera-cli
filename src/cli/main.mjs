...
export async function runCli({ argv = process.argv.slice(2), environment = process.env, output = process.stdout, errorOutput = process.stderr } = {}) {
  ...
  try {
    ...
    if (command === 'restore-artifact') { const root = argv[1]; if (!root || !argv.includes('--confirm')) { errorOutput.write('restore-artifact requires a path and --confirm\n'); return exitCodes.invalid; } const bundle = await client.lease(config.database, config.identity); emit({ ok: true, operation: command, data: await restoreArtifact({ root, client, bundle, confirm: true }) }); return 0; }
