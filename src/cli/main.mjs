...
export async function runCli({ argv = process.argv.slice(2), environment = process.env, output = process.stdout, errorOutput = process.stderr } = {}) {
  ...
  if (command === '--help' || command === undefined) { emit('elera-cli <health|ready|status|config-inspect|config-plan|config-apply|config-verify|metadata-status|metadata-init|metadata-verify|cluster-status|cluster-observations|cluster-quorum|cluster-plan|cluster-bootstrap|cluster-join|cluster-leave|cluster-recover|sql-smoke> [--json]'); return 0; }
  ...
  if (!['health', 'ready', 'status', 'config-inspect', 'config-plan', 'config-apply', 'config-verify', 'metadata-status', 'metadata-init', 'metadata-verify', 'cluster-status', 'cluster-observations', 'cluster-quorum', 'cluster-plan', ...Object.keys(lifecycleCommands), 'sql-smoke'].includes(command)) { errorOutput.write(`unknown command: ${command}\n`); return exitCodes.invalid; }
