export async function dispatchManagement({ command, client, controlClient, emit, args, options = {}, operationId, handlers }) {
  const calls = {
    ...
    'node-reset-data': () => handlers.nodeResetData({ client, emit, node: args[0], confirm: options.confirm, force: options.force === 'true' || options.force === true, disposition: options.disposition, idempotencyKey: operationId, dryRun: options['dry-run'] === true }),
    'node-resync': () => handlers.nodeResync({ client, emit, node: args[0], confirm: options.confirm, force: options.force === 'true' || options.force === true, idempotencyKey: operationId, dryRun: options['dry-run'] === true }),
