export async function dispatchManagement({ command, client, controlClient, emit, args, options = {}, operationId, handlers }) {
  const calls = {
    ...
    'node-reset-data': () => handlers.nodeResetData({ client, emit, node: args[0], confirm: options.confirm, force: options.force === 'true' || options.force === true, disposition: options.disposition, idempotencyKey: operationId, dryRun: options['dry-run'] === 'true' || options['dry-run'] === true }),
    'node-resync': () => handlers.nodeResync({ client, emit, node: args[0], confirm: options.confirm, force: options.force === 'true' || options.force === true, idempotencyKey: operationId, dryRun: options['dry-run'] === 'true' || options['dry-run'] === true }),
    'identity-create': () => handlers.identityCreate({ client, emit, application: args[0], database: args[1], identity: args[2], purpose: args[3], grants: options.grants ?? args[4] }),
    ...
    'token-create': () => handlers.tokenCreate({ client, emit, tokenName: args[0], application: args[1], identity: args[2], scopes: options.scopes ?? args[3] }),
