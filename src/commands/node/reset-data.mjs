export async function runNodeResetData({ client, emit, node, confirm, force = false, disposition = 'single-member-resync', idempotencyKey, dryRun = false, confirmationPrefix = 'RESET', endpointConfirmation = confirm }) {
  ...
  emit(await client.resetNodeData(node, { confirm: endpointConfirmation, force, recoveryDisposition: disposition, idempotencyKey }));
  return 0;
}

export async function runNodeResync(options = {}) {
  return runNodeResetData({ ...options, disposition: 'single-member-resync', confirmationPrefix: 'RESYNC', endpointConfirmation: `RESET ${options.node}` });
}
