...
test('requires a backup path for the backup list command', async () => { const error = stream(); const environment = { ELERA_API_ENDPOINT: 'http://supervisor', ELERA_API_TOKEN: 'token', ELERA_DATABASE: 'app', ELERA_IDENTITY: 'runtime' }; expect(await runCli({ argv: ['backup', 'list'], environment, output: stream(), errorOutput: error })).toBe(2); expect(error.value).toContain('backup-list requires a backup path'); });
test('rejects commands that are not part of the supported command tree', async () => { const error = stream(); const environment = { ELERA_API_ENDPOINT: 'http://supervisor', ELERA_API_TOKEN: 'token', ELERA_DATABASE: 'app', ELERA_IDENTITY: 'runtime' }; expect(await runCli({ argv: ['diagnostics', 'safety'], environment, output: stream(), errorOutput: error, dependencies: { createSupervisorClient: () => ({}) } })).toBe(2); expect(error.value).toContain('unknown command'); });
...
test('rejects a cluster plan without an action', async () => { const error = stream(); const environment = { ELERA_API_ENDPOINT: 'http://supervisor', ELERA_API_TOKEN: 'token', ELERA_DATABASE: 'app', ELERA_IDENTITY: 'runtime' }; expect(await runCli({ argv: ['cluster', 'plan'], environment, output: stream(), errorOutput: error })).toBe(2); expect(error.value).toContain('cluster-plan requires an action'); });
...
test('reports initialization and cold-bootstrap refusal without hiding the reason', async () => {
  ...
  const client = {
    ...
    coldBootstrapPlan: async () => ({ data: { eligible: false, reason: 'ambiguous seqno' } }),
  };
...
test('handles standalone planning responses without a data wrapper', async () => {
  const environment = { ELERA_API_ENDPOINT: 'http://supervisor', ELERA_API_TOKEN: 'token', ELERA_DATABASE: 'app', ELERA_IDENTITY: 'runtime' };
  const client = { initializationPlan: async () => ({ ok: true, database: 'elera_meta' }) };
  expect(await runCli({ argv: ['initialization', 'standalone', '--dry-run', '--json'], environment, output: stream(), errorOutput: stream(), dependencies: { createSupervisorClient: () => client } })).toBe(1);
});
...
test('dispatches recovery and routing administration commands', async () => {
  const environment = { ELERA_API_ENDPOINT: 'http://supervisor', ELERA_API_TOKEN: 'token' };
