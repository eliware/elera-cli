...
test('routes metadata initialization through the authenticated control client', async () => { const metadataInitialize = jest.fn(() => 0); const client = {}; const controlClient = {}; await dispatchManagement({ command: 'metadata-init', client, controlClient, emit: jest.fn(), args: [], handlers: { metadataInitialize } }); expect(metadataInitialize).toHaveBeenCalledWith({ client: controlClient, emit: expect.any(Function) }); });
