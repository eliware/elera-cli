...
test('creates and verifies a compressed logical backup', async () => {
  const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-backup-')); const release = jest.fn();
...
test('rejects empty backups and cleans up failed staging work', async () => {
  const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-backup-')); await expect(createBackup({ backupRoot: root, databases: ['mysql'] })).rejects.toThrow('No eligible');
...
test('reports missing backups, rotates old entries, and rejects invalid gzip', async () => {
  expect(await backupExists('/does/not/exist')).toBe(false); const removed = []; await rotateBackups({ backupRoot: 'root', keep: 2, list: async () => [{ name: '1' }, { name: '2' }, { name: '3' }], remove: async (path) => removed.push(path) }); expect(removed).toEqual(['root\\2', 'root\\1']);
  const read = async () => Buffer.from('not gzip'); await expect(verifyBackup({ backupPath: 'x', databases: ['app'], read })).rejects.toThrow();
});

test('dumpToGzip reports child process failure', async () => { await expect(dumpToGzip(process.execPath, ['-e', "process.stderr.write('bad'); process.exit(2)"], join(process.env.TEMP ?? process.env.TMP, 'elera-dump.gz'))).rejects.toThrow('bad'); });
test('dumpToGzip uses the exit code when the child has no stderr', async () => { const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-dump-')); await expect(dumpToGzip(process.execPath, ['-e', 'process.exit(2)'], join(root, 'empty.gz'))).rejects.toThrow('dump exited with 2'); });

test('verifies the databases listed by the backup when no filter is supplied', async () => {
  const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-backup-'));
...
test('handles a backup failure before staging is created', async () => {
  const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-backup-')); const file = join(root, 'not-a-directory'); await writeFile(file, 'x');
