...
test('restores metadata, accounts, and application dumps while skipping system schemas', async () => {
  ...
  await writeFile(join(root, 'SUPERVISOR-METADATA.json'), JSON.stringify({ databases: ['app'], identities: ['runtime'], accounts: [{ user: 'u' }] }));
  await writeFile(join(root, 'DATABASES'), 'mysql\napp\nsys\n');
  await writeFile(join(root, 'app.sql.gz'), gzipSync('sql'));
  const client = { restoreMetadataApply: jest.fn(async () => {}), restoreAccountsApply: jest.fn(async () => {}) };
