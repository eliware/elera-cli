...
test('creates, verifies, and restore-verifies a bundle backup with metadata', async () => {
  const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-adapter-')); const dump = async (_binary, _args, path) => (await import('node:fs/promises')).writeFile(path, gzipSync('sql')); const rotate = async () => {};
...
test('creates a bundle backup without a metadata sidecar', async () => {
  const root = await mkdtemp(join(process.env.TEMP ?? process.env.TMP, 'elera-adapter-')); const dump = async (_binary, _args, path) => (await import('node:fs/promises')).writeFile(path, gzipSync('sql'));
